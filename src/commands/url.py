import click
from ..util import parse_date, pull_url_id, pull_comments_page, add_click_options, COMMENT_PAGE_OPTIONS
...
def command(url, sort, after_id, after_time, max):
    ...
    page = 1
    total_emitted = 0
    while len(page_comments := pull_comments_page(s=sort, p=page, v="begin", cpp=350, uid=url_id)) > 0:
